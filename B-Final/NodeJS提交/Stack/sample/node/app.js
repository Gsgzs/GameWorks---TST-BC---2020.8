...
function OptimalRoute() {
    ...
    console.log('剩余机会次数：' + (environment.chance - 1));
    return {
        speed: speed_merit,
        angle: angle_merit
    }
...
function noPortionRoute(obArr) {
    ...
    // console.log(eOOArr);
    // 偏移范围
    let offsetX = environment.player.radius + 30;
    let offsetY = environment.player.radius + 40;
    // let offset = 0;
    let ii = 0;
    let resultArr = [];
    while (resultArr.length == 0 && sortDistance.length - 1 != ii) {
        ...
        if (endArr[0][1] >= endArr[1][1]) { //左高右低
            let endX = endArr[1][0] + (offsetX);
            let endY = endArr[1][1] - (offsetY);
            let endSize = [endX, endY];
            // console.log(endArr);
            for (let i = 0; i < environment.obstacles.length; i++) {
                if (judgeIntersect(eOOArr[i][0], eOOArr[i][1], endSize, playerXY)) {
                    flag = false;
                    break;
                };
            }
            if (flag) {
                resultArr.push(twoDistance(playerXY, endSize) * 0.01875, getAngleByPoint([playerXY, endSize]))
            } else {
                flag = true;
                let endX = endArr[0][0] - (offsetX);
                let endY = endArr[0][1] + (offsetY);
                let endSize = [endX, endY];
                // console.log(endArr);
                for (let i = 0; i < environment.obstacles.length; i++) {
                    if (judgeIntersect(eOOArr[i][0], eOOArr[i][1], endSize, playerXY)) {
...
function getUsefulPosition() {
    ...
    for (let i = 0; i < sortDistance.length; i++) {
        ...
        while (whileflag <= 15000 && !flag(sortDistance[i], randomX, randomY)) {
            whileflag++;
            randomX = Math.floor(Math.random() * maxX);
            randomY = Math.floor(Math.random() * maxY);
            console.log('计数器：' + whileflag);
        };
        console.log('===== 通道1 =====');
        if (whileflag <= 15000) {
            console.log('===== 通道2 [成功] =====');
